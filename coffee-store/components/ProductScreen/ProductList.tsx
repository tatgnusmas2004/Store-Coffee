...
const ProductList = ({ keyword }) => {
  ...
  const fetchProducts = async () => {
    ...
    const url = keyword
      ? `${config.SEARCH_PRODUCTS_URL}`
      : `${config.PRODUCTS_URL}/page`;
    const params = { pdName: keyword || undefined, page, size: 6 };
    ...
    try {
      ...
      if (page === 0) {
        setFoods(newProducts); // Reset dữ liệu nếu là trang đầu tiên
      } else {
        setFoods((prevFoods) => [...prevFoods, ...newProducts]);
      }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListFooterComponent={
          loading ? <ActivityIndicator size="large" color="#e99211" /> : null
        }
      />
